@php
    $categories = App\Models\Category::with('subs')->withCount('subs')->get();
@endphp
<div class="col-xl-3">
        ...
        <div id="woocommerce_product_categories-4" class="widget woocommerce widget_product_categories widget-toggle">
            ...
            <ul class="product-categories">
                ...
                <li class="cat-item cat-parent">
                    @if($category->slug == 'skin-care' && $category->status !=2)
                      <a href="{{route('front.category', $category->slug)}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link text-capitalize" id="cat">{{ $category->name }} <span class="count"></span></a>
                    @else
                    <a href="{{route('front.how-to-use', $category->slug)}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link text-capitalize" id="cat">{{ $category->name }} <span class="count"></span></a>
                    @endif
                ...
                <div class="product-style-2 owl-carousel owl-nav-hover-primary nav-top-right single-carousel dot-disable product-list e-bg-white">
                    ...
                    <div class="item">
                        <div class="row row-cols-1">
                            ...
                            <div class="col mb-1">
                                <div class="product type-product pro-cover-area">
                                    <div class="product-wrapper pro-detect-area">
                                        ...
                                        <div class="product-info ">
                                            <h3 class="product-title font-info"><a href="{{ route('front.product', $prod['slug']) }}">{{ ucfirst(mb_strtolower(mb_strimwidth($prod['name'], 0, 30, '...'))) }}</a></h3>
