      ...
      <div class="col-lg-12">
        <div class="product-description">
          <div class="body-area" id="modalEdit">
            ...
            <form id="geniusformdata" action="{{route('vendor-order-update',$data->id)}}" method="POST" enctype="multipart/form-data">
              {{csrf_field()}}
              ...
              <div class="row">
                ...
                <div class="col-lg-7">
                  <select name="status" required="" id="myDiv">
                    ...
                    <option value="declined" id="dropDownId" {{ $data->status == "declined" ? "selected":"" }}>{{ __('Declined') }}</option>
                  </select>
                </div>
              </div>
              <div class="row">
              ...
              <div class="row">
                ...
                <div class="col-lg-7">
                  <button class="addProductSubmit-btn mybtn8" type="submit">{{ __('Save') }}</button>
                </div>
              </div>
            </form>
          </div>
...
<script>
  ...
  $(".mybtn8").click(function(){
         var cancel = $('#dropDownId').val();
         $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                url:mainurl+"/vendor/cancelWaybill/{{$data->id}}",
                type: 'post',
                data: { cancel: cancel },
