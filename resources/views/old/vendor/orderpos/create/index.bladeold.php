   ...
   <div class="row">
      <div class="col-lg-12">
         ...
         <ul class="links">
            ...
            <li>
               <a href="{{ route('vendor-order-create') }}">{{ __('Create An Order') }}</a>
            </li>
         </ul>
      </div>
   </div>
...
<div class="add-product-content1 add-product-content2">
    <div class="row">
      <div class="col-lg-12">
        ...
        <form action="{{route('vendor.order.create.view')}}" method="POST">
            @csrf
                ...
                <div class="row">
                  ...
                  <div class="col-lg-9" >
                    ...
                    <div class="py-4 px-4 my-2 mx-4 border">
                      ...
                      <div class="row">
                        <div class="col-lg-4">
                            <label for="phone_number">Select Existing Customer</label>
                            <div class="input-group mb-2">
                              <select name="user_id" class="form-control order_create_user" id="order_create_user">
                                <option value="">Select User</option>
                                @foreach(App\Models\User::where('seller_id',Auth::user()->id)->get() as $user)
...
(function($) {
  ...
  var table = $('#order-geniustable').DataTable({
      ...
      ajax: '{{ route('vendor-order-product-datatables') }}',
      columns: [
              { data: 'name', name: 'name' },
...
$('#product-show').html('').load(mainurl+"/vendor/order/create/product-show/"+$(this).attr('data-href'),function(response, status, xhr){
          if(status == "success")
          {
              if(admin_loader == 1)
              {
