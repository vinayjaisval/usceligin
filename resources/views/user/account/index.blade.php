    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
      ...
      <div class="lg:col-span-9">
        ...
        <div id="content-purchases" class="tab-content hidden">
          ...
          {{-- Sub-tabs: Orders | Buy Again | Cancel Order | Refund Request --}}
          <div class="border-b border-gray-200 dark:border-gray-700 mb-4 overflow-x-auto">
            <nav class="flex gap-6 min-w-max" role="tablist" aria-label="Purchase history tabs">
              ...
              <button type="button" role="tab" aria-selected="false" aria-controls="cancel-subtab-content"
                onclick="switchOrderSubTab('cancel')" id="subtab-cancel"
              ...
              <button type="button" role="tab" aria-selected="false" aria-controls="refund-subtab-content"
                onclick="switchOrderSubTab('refund')" id="subtab-refund"
                class="order-subtab pb-3 text-sm font-semibold border-b-2 border-transparent text-neutral-500 dark:text-neutral-400 hover:text-primary-800 dark:hover:text-primary-400 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-offset-2">
                Refund Request
              </button>
            ...
            @if($orders->count() > 0)
              <div class="space-y-5">
                @foreach($orders as $order)
                  @include('user.account._order-card', ['order' => $order])
                @endforeach
              </div>
            @else
              {{-- Empty State --}}
          ...
          {{-- Cancel Order Sub-tab Content --}}
          <div id="cancel-subtab-content" class="hidden">
            @php
              $cancelOrders = $orders->filter(fn($o) => in_array($o->status, ['pending', 'declined']));
              $pendingCount = $cancelOrders->where('status', 'pending')->count();
            @endphp
            @if($cancelOrders->count() > 0)
              @if($pendingCount > 0)
                <div class="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 text-sm text-amber-800 dark:text-amber-200">
                  <span class="material-icons-outlined text-base align-middle mr-1" aria-hidden="true">info</span>
                  Orders can only be cancelled while their status is <strong>Pending</strong>. Once processing begins, cancellation is no longer available.
                </div>
              @endif
              <div class="space-y-5">
            ...
            @else
              <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-8 lg:p-12 text-center">
                <div class="max-w-md mx-auto">
                  <div class="w-20 h-20 bg-green-50 dark:bg-green-900/20 rounded-full flex items-center justify-center mx-auto mb-5">
                    <span class="material-icons-outlined text-5xl text-green-500 dark:text-green-400" aria-hidden="true">check_circle</span>
                  </div>
                  <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">No cancellations</h3>
                  <p class="text-sm text-gray-600 dark:text-gray-400">
                    You have no orders pending cancellation or previously cancelled.
                  </p>
                </div>
              </div>
            @endif
          ...
          {{-- Refund Request Sub-tab Content --}}
          <div id="refund-subtab-content" class="hidden">
            @php
              $refundWindow = config('order.refund_window_days', 5);
              $refundOrders = $orders->filter(function($o) use ($refundWindow) {
                if (in_array($o->status, ['refund_requested', 'refunded'])) return true;
                if ($o->status === 'completed') {
                  $daysLeft = max(0, $refundWindow - (int) $o->updated_at->diffInDays(now()));
                  return $daysLeft > 0;
                }
                return false;
              });
            ...
            @if($refundOrders->count() > 0)
              <div class="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 text-sm text-blue-800 dark:text-blue-200">
                <span class="material-icons-outlined text-base align-middle mr-1" aria-hidden="true">info</span>
                Refund requests can be submitted within <strong>{{ $refundWindow }} days</strong> of delivery. Our team reviews requests within 1–2 business days.
              </div>
              <div class="space-y-5">
              ...
              <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-8 lg:p-12 text-center">
                <div class="max-w-md mx-auto">
                  <div class="w-20 h-20 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-5">
                    <span class="material-icons-outlined text-5xl text-gray-400 dark:text-gray-500" aria-hidden="true">assignment_return</span>
                  </div>
                  <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">No refund requests</h3>
                  <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">
                    You have no active refund requests or orders eligible for a refund at this time.
                  </p>
                  <a href="{{ route('front.return-refund-policy') }}"
                    class="text-sm text-primary-700 dark:text-primary-400 hover:underline">
...
<script>
  ...
  // Switch order sub-tabs (Orders / Buy Again / Cancel Order / Refund Request)
  const ORDER_SUBTABS = ['orders', 'buyagain', 'cancel', 'refund'];
  function switchOrderSubTab(tab) {
    ...
    ORDER_SUBTABS.forEach(t => {
      document.getElementById(t + '-subtab-content').classList.toggle('hidden', t !== tab);
    });
  }
