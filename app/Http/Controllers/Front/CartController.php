...
class CartController extends FrontBaseController
{
    ...
    //             $cart->add($variation->id, $item['quantity'], [
    //                 'price' => $price,
    ...
    public function multiAddToCart(Request $request)
    {
        ...
        Session::put('cart', $cart);
        Session::put('celigin-combo', 'Celigin Combo');
        $messages = [];
        if ($addedProducts) {
            $messages[] = __('The following products were added:') . ' ' . implode(', ', $addedProducts) . '.';
        }
        if ($outOfStockProducts) {
            $messages[] = __('Out of stock products:') . ' ' . implode(', ', $outOfStockProducts) . '.';
        }
        // dd(Session::get('cart'));
