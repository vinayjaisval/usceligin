...
class VendorController extends VendorBaseController
{
    ...
    public function index()
    {
        ...
        for($i = 0; $i < 30; $i++) {
            ...
            $data['sales'] .=  "'".Order::where('seller_id','=',$this->user->id)->where('status','=','completed')->whereDate('created_at', '=', date("Y-m-d", strtotime('-'. $i .' days')))->count()."',";
        }
        $data['pproducts'] = Product::where('user_id','=',$this->user->id)->latest('id')->take(6)->get();
        $data['pproducts'] = Product::latest('id')->take(6)->get();

        // $data['rorders'] = VendorOrder::where('user_id','=',$this->user->id)->latest('id')->take(10)->get();
        $data['rorders'] = Order::where('seller_id','=',$this->user->id)->latest('id')->take(10)->get();
      
      
        $data['user'] = $this->user;  
        $data['totalproducts'] = Product::where('status',1)->latest('id')->get();

        // $data['pending'] = VendorOrder::where('user_id','=',$this->user->id)->where('status','=','pending')->get(); 
        $data['pending'] = Order::where('seller_id','=',$this->user->id)->where('status','=','pending')->get(); 

        // $data['processing'] = VendorOrder::where('user_id','=',$this->user->id)->where('status','=','processing')->get(); 
        $data['processing'] = Order::where('seller_id','=',$this->user->id)->where('status','=','processing')->get(); 
       
        // $data['completed'] = VendorOrder::where('user_id','=',$this->user->id)->where('status','=','completed')->get(); 
        $data['completed'] = Order::where('seller_id','=',$this->user->id)->where('status','=','completed')->get(); 
    ...
    {
        $users = User::where('seller_id' , '=', $this->user->id)->orderBy('id', 'desc')->get();
        return view('vendor.mlm-users', compact('users'));
    }
