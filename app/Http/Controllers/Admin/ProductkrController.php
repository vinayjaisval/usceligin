...
class ProductkrController extends AdminBaseController
{
    //*** JSON Request
    public function datatables(Request $request)
    {
        if ($request->type == 'all') {
            // $datas = Product::whereProductType('normal')->latest('id')->whereor('canvert_status_lan',[0,1])->get();

            $datas = Product::where('product_type', 'normal')
                ->whereIn('canvert_status_lan', [0, 1])
                ->latest('id')
                ->get();
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('action', function (Product $data) {
                $catalog = $data->type == 'Physical' ? ($data->is_catalog == 1 ? '<a href="javascript:;" data-href="' . route('admin-prod-catalog-kr', ['id1' => $data->id, 'id2' => 0]) . '" data-toggle="modal" data-target="#catalog-modal" class="delete"><i class="fas fa-trash-alt"></i> ' . __("Remove Catalog") . '</a>' : '<a href="javascript:;" data-href="' . route('admin-prod-catalog-kr', ['id1' => $data->id, 'id2' => 1]) . '" data-toggle="modal" data-target="#catalog-modal"> <i class="fas fa-plus"></i> ' . __("Add To Catalog") . '</a>') : '';
                return '<div class="godropdown"><button class="go-dropdown-toggle"> ' . __("Actions") . '<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="' . route('admin-prod-edit-kr', $data->id) . '"> <i class="fas fa-edit"></i> ' . __("Edit") . '</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="' . $data->id . '"><i class="fas fa-eye"></i> ' . __("View Gallery") . '</a>' . $catalog . '<a data-href="' . route('admin-prod-feature', $data->id) . '" class="feature" data-toggle="modal" data-target="#modal2"> <i class="fas fa-star"></i> ' . __("Highlight") . '</a><a href="javascript:;" data-href="' . route('admin-prod-delete-kr', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> ' . __("Delete") . '</a></div></div>';
            })
            ->rawColumns(['name', 'status', 'action'])
            ->toJson(); //--- Returning Json Data To Client Side
    }

    function translateInChunks($content, $targetLang = 'ko', $chunkSize = 4900)
    {
        $chunks = str_split($content, $chunkSize);
        $translatedContent = '';
        foreach ($chunks as $chunk) {
            try {
                $translatedContent .= GoogleTranslate::trans($chunk, $targetLang);
            } catch (\Exception $e) {
                dd('Translation Error: ' . $e->getMessage());
            }
        }
    ...
    {
        return view('admin.productkr.catalog');
    }
    public function index()
    {
        return view('admin.productkr.index');
    }

    public function types()
    {
        return view('admin.productkr.types');
    }

    public function deactive()
    {
        return view('admin.productkr.deactive');
    }

    public function productsettings()
    {
        return view('admin.productkr.settings');
    }
    ...
    public function create($slug)
    {
        ...
        $sign = $this->curr;
        if ($slug == 'physical') {
            return view('admin.productkr.create.physical', compact('cats', 'brand','sign'));
        } else if ($slug == 'digital') {
            return view('admin.productkr.create.digital', compact('cats', 'sign'));
        } else if (($slug == 'license')) {
            return view('admin.productkr.create.license', compact('cats', 'sign'));
        } else if (($slug == 'listing')) {
            return view('admin.productkr.create.listing', compact('cats', 'sign'));
    ...
    public function import()
    {
        ...
        $cats = Category::all();
        $sign = $this->curr;
        return view('admin.productkr.productcsv', compact('cats', 'sign'));
    }
   ...
   public function importSubmit(Request $request)
    ...
    public function edit($id)
    {
        ...
        if ($data->type == 'Digital') {
            return view('admin.productkr.edit.digital', compact('cats', 'data', 'sign'));
        } elseif ($data->type == 'License') {
            return view('admin.productkr.edit.license', compact('cats', 'data', 'sign'));
        } elseif ($data->type == 'Listing') {
            return view('admin.productkr.edit.listing', compact('cats', 'data', 'sign'));
        } else {
            return view('admin.productkr.edit.physical', compact('cats','brand', 'data', 'sign'));
        }
    ...
    public function update(Request $request, $id)
    {
     ...
     if($data->canvert_status_lan ==0){
        $data1 = new Product(); // Create a new Product instance
        $input['name'] = isset($input['name']) ? GoogleTranslate::trans($input['name'], 'ko') : null;
        $input['details'] = isset($input['details']) ? GoogleTranslate::trans($input['details'], 'ko') : null;
        $inputDetails = $input['benefits']; 
        $input['benefits'] =  isset($input['benefits']) ? $this->translateInChunks($inputDetails): null;
        $input['how_to_use'] = isset($input['how_to_use']) ? GoogleTranslate::trans($input['how_to_use'], 'ko') : null;
        $input['ingredients'] = isset($input['ingredients']) ? GoogleTranslate::trans($input['ingredients'], 'ko') : null;
        $input['meta_description'] = isset($input['meta_description']) ? GoogleTranslate::trans($input['meta_description'], 'ko') : null;
        $input['meta_tag'] = isset($input['meta_tag']) ? GoogleTranslate::trans($input['meta_tag'], 'ko') : null;
        $input['user_id'] = $data->user_id ?? null;
        $input['status'] = $data->status ?? null;
        $input['views'] = $data->views ?? null;
        $input['product_type'] = $data->product_type ?? null;
        $input['product_condition'] = $data->product_condition ?? null;
        $input['is_meta'] = $data->is_meta ?? null;
        $input['featured'] = $data->featured ?? null;
        $input['best'] = $data->best ?? null;
        $input['top'] = $data->top ?? null;
        $input['hot'] = $data->hot ?? null;
        $input['latest'] = $data->latest ?? null;
        $input['big'] = $data->big ?? null;
        $input['trending'] = $data->trending ?? null;
        $input['sale'] = $data->sale ?? null;
        $input['is_discount'] = $data->is_discount ?? null;
        $input['is_catalog'] = $data->is_catalog ?? null;
        $input['catalog_id'] = $data->catalog_id ?? null;
        $input['preordered'] = $data->preordered ?? null;
        $input['stock_check'] = $data->stock_check ?? null;
        $data1->fill($input); // Fill with input data
        $data1->slug = Str::slug($input['name'], '-') . '-' . strtolower($input['sku']); // Generate slug
        $data1->canvert_status_lan = 1; // Set the status to 1
        $data1->save(); // Save the new product
        $data->canvert_status_lan = 3;
        $data->save();
        $msg = __("Product added successfully.") . '<a href="' . route('admin-prod-index') . '">' . __("View Product List.") . '</a>';
        return response()->json($msg);
     }
     else{
        $data->update($input);
    ...
    {
        $data = Product::findOrFail($id);
        return view('admin.productkr.highlight', compact('data'));
    }
