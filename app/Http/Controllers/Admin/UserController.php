...
class UserController extends AdminBaseController
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ->addColumn('action', function (User $data) {
                ...
                $affiliate = $data->reffered_times < 3 ? '<a href="' . route('admin-user-make-affiliate', $data->id) . '" class="make-affiliate" >
                                            <i class="fas fa-dollar-sign"></i>' . __("Make Affiliate User") .
                    '</a>' : '';
    ...
    {
        $datas = User::latest('id')-> where('reffered_times', 3)->get();
        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
            ->addColumn('action', function (User $data) {
                $class = $data->ban == 0 ? 'drop-success' : 'drop-danger';
                ...
                $affiliate = $data->reffered_times < 5 ? '<a href="' . route('admin-user-make-affiliate', $data->id) . '" class="make-affiliate" >
                                            <i class="fas fa-dollar-sign"></i>' . __("Make Affiliate User") .
                    '</a>' : '';
                return '<div class="action-list">
                                            '
    ...
    {
        return view('admin.user.affiliate-user');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $data = $request->all();
        $data['affilate_code'] = md5($request->name . $request->email);
        if ($file = $request->file('photo')) {
            $name = \PriceHelper::ImageCreateName($file);
            $file->move('assets/images/users', $name);
            if ($user->photo != null) {
    ...
    public function mlm_users()
    {
        ...
        $users = User::latest('id')-> where('reffered_times', 3)->get();
    ...
    // }
    public function datatablesinfluencer()
    {
        $datas = Influencer::oldest('id')->get();
        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
            ->addColumn('sync_action', function ($influencer) {
                $exists = DB::table('users')->where('email', $influencer->email)->exists();
                return $exists ? '' : url('admin/influencer/sync-to-user/' . $influencer->id);
            })
            ->addColumn('sl', function (Influencer $data) {
                $id = 1;
                return $id++;
            })
            ->toJson(); //--- Returning Json Data To Client Side
    ...
    public function syncToUser($id)
    {
        ...
        if (!$user) {
            ...
            $user->affilate_code = md5($data->name . $data->email);
            $user->is_vendor = 0;
            $user->status = 1;
            $user->save();
            $data = [
                'to' => $user->email,
                //'to' => 'ranjeetmaurya2033@gmail.com',
                'subject' => "Account Created",
                'body' => 'Dear ' . $user->name . ',<br><br>
                    Your account has been created successfully, your login details are as follows:<br>
                    Username: ' . $user->email . '<br>
                    Password: ' . $password . '<br><br>
                    Please login to your account by clicking <a href="' . route('user.login') . '">here</a>.<br><br>
                    Thank you,<br>
            ...
            $msg = __('Successfully Created User');
        } else {
            $msg = __('Already User Created');
        }
        return redirect()->back()->with('success', $msg);
    }

    public function makeInfluencerAffilated($id)
    {
        $make_affilate = User::find($id);
        $make_affilate->reffered_times = 3;
        $make_affilate->update();
        $msg = __('User Affilated Successfully');
        return redirect()->back()->with('success', $msg);
    }
}
